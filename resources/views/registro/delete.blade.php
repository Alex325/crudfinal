@extends('layouts.main')

@section('content')
<div class="add-foto-form-container">
    <form action="{{route('registro.destroy', $reg->id)}}" method="post" class="add-reg-form">
        @csrf
        @METHOD("DELETE")
        <div class="input-container input-container-titulo">
            <label class="label titulo" for="titulo-input">Título</label>
            <input type="text" value="{{ $reg->titulo }}" name="titulo" id="titulo-input" class="input" disabled required>
        </div>
        <div class="input-container input-container-frase">
            <label class="label frase" for="frase-input">Frase do dia</label>
            <input type="text" value="{{ $reg->frase_do_dia }}" name="frase" id="frase-input" class="input" disabled required>
        </div>
        <div class="input-container input-container-texto">
            <label class="label text" for="texto-input">Texto</label>
            <textarea name="texto" id="texto-input" class="input" disabled required>{{ $reg->texto }}</textarea>
        </div>
        <div class="input-container input-container-cor">
            <label class="label cor" for="cor-input">Cor</label>
            <input type="color" value="{{ $reg->cor }}" name="cor" id="cor-input" class="input" disabled required>
        </div>
        <div class="input-container input-container-botoes">
            <button type="submit" class="botao submit">Excluir</button>
            <button type="reset" class="botao reset">Limpar</button>
            <input type="hidden" name="enviado" value="enviado">
        </div>
    </form>
</div>
</div>
@endsection

@section('style')
<link rel="stylesheet" href="{{ asset('css/addReg.css') }}">
@endsection